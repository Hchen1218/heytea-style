...
`source signal → creature role → abstraction method → body attachment or region`
...
- facial-mark count, placement, line shape, hierarchy, and friendly emotion. Eyes must occupy a readable eye level and any mouth must read below them at 120–140 px; single-eyed or mouthless faces remain allowed only when their remaining facial semantics are unmistakable. For the face and every limb, declare where the stroke meanders or bends and which within-stroke pressure change, thickness jump, dry break, retracing, overshoot, or open join keeps it visibly handmade;
