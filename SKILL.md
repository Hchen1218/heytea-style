...
Generate one best complete-color `flavor-monster` identity on white or warm white. The photo controls flavor DNA and body topology; `examples/desktop-pet/pink-green-flavor-monster-v3/preview.png` controls only HEYTEA-like cuteness and the relationship between an unoutlined color body and sparse black doodle features. It must not supply facial-mark type, count, placement, or hierarchy, nor limb attachment points, lengths, proportions, or resting pose. Use the dry-media board for body material and the thin-stroke board for the face and exactly two arms plus two legs. Before generation, declare how the new face system and limb geometry differ from the example; a palette-swapped example skeleton fails.

Every `flavor-monster` must translate non-color source DNA into at least one source-derived silhouette feature that reads at 120–140 px, normally one or two abstract ear-like fins, horns, crests, spines, tail buds, antennae, or comparable protrusions. Color, texture, spots, or internal marks alone do not satisfy this monster-structure gate. Keep each feature integrated into the unoutlined body topology, abstract rather than a literal ingredient or realistic organ, and visibly separate from the fixed inventory of exactly two black-line arms plus two black-line legs.

Before generation, assign each major body region to loose colored-pencil sweeps, a few broad wax-crayon arcs or separated blocks, active paper-white, or an occasional faded edge. Require significant irregular paper-white gaps inside the body, not only white background or paper grain, and do not let every region share one dense texture. In the model instruction, require long loose lightly layered pencil travel; sparse broad open crayon marks; occasional missed, faded, or slightly crossed boundaries; and face and limb strokes that meander along their full length with within-stroke pressure and thickness changes, dry breaks, retracing, overshoots, or open joins. Reject uniform digital grain, dense tiny strokes or loops, crosshatching, smooth Bézier curves, constant-width monoline, and a clean vector skeleton with superficial wobble.

Inspect the identity at full size and 120–140 px. At both sizes, the internal paper-white must read as deliberate material, the major color zones must not collapse into noise, and the black face plus all four limbs must remain legible. If the body is uniformly filled, keep its topology, DNA, face, and limbs locked and repair only the material axis; use the smudged-structure board only for that focused repair. If the black marks are smooth or constant-width, keep the body and color zones locked and redraw only the existing face and limb marks. Then ask the user to approve the identity or request another focused revision. Do not generate a poster, motion, runtime assets, or package before explicit identity approval. Approval establishes the canonical identity shared by later fusion-poster and desktop-pet branches.
